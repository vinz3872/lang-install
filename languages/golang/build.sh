...
# go vs golang...
  ...
  echo $(docker run -it --rm --network=host -v "$PWD:$PWD" -w $PWD -u `id -u` path_$bin_name:$version bash -c "which go | xargs dirname")
  # declare -a bins=$(docker run -it --rm --network=host -v "$PWD:$PWD" -w $PWD -u `id -u` path_$bin_name:$version ls -I "*.sh" /usr/local/go/bin)
#   for i in $bins; do
